...
const GamelogicSimulation = require('./gamelogic-simulation');
...
class Emulator {
  constructor(romObject) {
    ...
    const triggerSwitchFunction = this.cpuBoard.setInput.bind(this.cpuBoard);
    this.gamelogicSimulation = GamelogicSimulation.initialiseGameLogic(triggerSwitchFunction, romObject.gameLogic);
  }
  ...
  getUiState() {
    ...
    if (uiState && uiState.asic && uiState.asic.wpc && uiState.asic.wpc.solenoidState) {
      this.gamelogicSimulation.updateSolenoidState(uiState.asic.wpc.solenoidState);
    }
  ...
  executeCycle(ticksToRun = 500, tickSteps = 4) {
    this.gamelogicSimulation.executeCycle();
  ...
  setInput(value) {
    ...
    this.gamelogicSimulation.triggerInputSwitch(value);
  }
  ...
  reset() {
    ...
    this.startTime = Date.now();
    this.cpuBoard.reset();
    this.gamelogicSimulation.reset();
  }
